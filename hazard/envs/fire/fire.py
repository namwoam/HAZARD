...
import numpy as np
from hazard.envs.fire.manager import FireObjectManager
from hazard.envs.fire.fire_utils import *
from hazard.envs.fire.object import FireStatus, AgentStatus
...
def CHOUKA(T, constants=default_const):
    ...
    if T <= constants.CHOUKA_THRESHOLD_PROB_INCREASE:
        ...
        return min(
            ...
            + (1 - start_prob)
            / (
                constants.CHOUKA_THRESHOLD_UPPER
                - constants.CHOUKA_THRESHOLD_PROB_INCREASE
            )
...
def box_overlap(pos1, size1, pos2, size2):
    ...
    return np.all(pos1 + size1 / 2 > pos2 - size2 / 2) and np.all(
        pos1 - size1 / 2 < pos2 + size2 / 2
    )
...
class FireController(Controller):
    ...
    def add_fire(self, position, object_id=None, scale=None):
        ...
        self.commands.append(
            self.get_add_visual_effect("fire", effect_id=object_id, position=position)
        )
        self.commands.append(
            {
    ...
    def add_fire_object(self, idx):
        ...
        self.commands.append(
            {"$type": "parent_visual_effect_to_object", "object_id": idx, "id": new_idx}
        )
    ...
    def extinguish_fire_object(self, obj_idx):
        ...
        self.manager.objects[self.fire_info[obj_idx].fire_id].temperature = (
            self.constants.ROOM_TEMPERATURE
        )
        pass
    ...
    def add_agent(self, idx, pos):
        self.manager.add_object(
            AgentStatus(
                idx, constants=self.constants, position=pos, size=None, endurance=10000
            )
        )
    ...
    def candidate_fire_overlap(self, pos, size):
        ...
        for idx in self.fire_info:
            if box_overlap(
                ...
                self.constants.FIRE_SPREAD_SIZE,
            ):
                return True
        return False
    ...
    def try_spread(self, idx):
        if (
            idx in self.manager.objects
            and np.abs(self.manager.objects[idx].position).sum() > 50
        ):
            return
        ...
        if self.RNG.uniform() < spread_prob:
            spread_dir = self.fire_info[idx].spread_dirs[
                self.RNG.choice(len(self.fire_info[idx].spread_dirs))
            ]
            self.fire_info[idx].spread_dirs.remove(spread_dir)
            spread_dir = np.array([spread_dir[0], 0, spread_dir[1]])

            pos = (
                self.manager.objects[idx].position * np.array([1, 0, 1])
                + spread_dir
                * (
                    self.constants.FIRE_SPREAD_SIZE
                    + self.manager.objects[self.fire_info[idx].fire_id].size
                )
                * 0.5
            )
            if not self.candidate_fire_overlap(pos, self.constants.FIRE_SPREAD_SIZE):
                self.fire_candidate[self.get_unique_id()] = (
                    pos,
                    self.constants.FIRE_SPREAD_SIZE,
                )
    ...
    def fire_step(self, resp):
        ...
        for idx in self.manager.objects_stop_burning:
            ...
            self.commands.append(
                {"$type": "destroy_visual_effect", "id": self.fire_info[idx].fire_id}
            )
        ...
        for i in range(len(resp) - 1):
            ...
            if r_id == "over":
                ...
                if (
                    idx in self.fire_candidate
                    and o.get_object_ids().size < 3
                    and o.get_env() == False
                ):
        ...
        if self.frame_count % self.update_fire_per_frame == 0:
            ...
            for idx in self.fire_info:
                if (
                    self.fire_info[idx].scale < self.constants.FIRE_FINAL_SCALE
                    and not self.fire_info[idx].extinguishing
                ):
                    # expand large enough before spreading
                    self.fire_info[idx].scale += self.constants.FIRE_SCALE_STEP
                    ratio = self.fire_info[idx].scale / (
                        self.fire_info[idx].scale - self.constants.FIRE_SCALE_STEP
                    )
                    self.commands.append(
                        {
                    ...
                    self.fire_info[idx].scale
                    >= self.constants.FIRE_INIT_SCALE + self.constants.FIRE_SCALE_STEP
                ):
