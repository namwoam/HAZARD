...
class FireAgentController(FireController):
    ...
    def init_scene(self, setup: SceneSetup):
                ...
                if tp == "add_object":
                    ...
                    if name in temperature_record:
                        self.manager.add_object(
                            ObjectStatus(
                                idx=idx,
                                temperature_threshold=temperature_record[name],
                                position=pos,
                            )
                        )
                    else:
        ...
        if not self.record_only and len(self.extinguishers) == 0:
            ...
            self.manager.add_object(
                ObjectStatus(
                    idx=self.container_id, inflammable=False, temperature_threshold=3000
                )
            )
    ...
    def add_extinguish_water(self, fire_pos):
        ...
        position = {
            "x": float(
                self.agents[0].dynamic.transform.position[0]
                + rotate[0] * extinguisher_distance_to_agent
            ),
            "y": float(self.agents[0].dynamic.transform.position[1]) + 0.7,
            "z": float(
                self.agents[0].dynamic.transform.position[2]
                + rotate[2] * extinguisher_distance_to_agent
            ),
        }
        rotation = {"x": 0, "y": 90, "z": 0}
        self.obi.create_fluid(
            fluid="water",
            shape=CubeEmitter(),
            object_id=object_id,
            position=position,
    ...
    def do_extinguish(self, target: np.ndarray):
        for idx, fire in self.fire_info.items():
            ...
            if (
                np.linalg.norm(self.manager.objects[fire_id].position - target)
                < self.constants.EXTINGUISH_RADIUS
            ):
                if idx == fire_id:
    ...
    def _obs(self, agent_idx: int = 0):
        ...
        log_temp = self.get_temperature_observation(
            agent_idx, width=self.screen_size, height=self.screen_size
        )
        log_temp = np.log(
            log_temp.reshape((1, self.screen_size, self.screen_size)).astype(np.float32)
        )
        obs["raw"] = dict(rgb=rgb, depth=depth, log_temp=log_temp, seg_mask=seg_mask)
