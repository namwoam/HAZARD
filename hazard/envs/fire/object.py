...
class ObjectStatus:
    def __init__(
        ...
        inflammable: bool = True,
        state: int = None,
        burning_time: int = 300,
        burning_time_left: int = 0,
        temperature: float = 20.0,
        temperature_threshold: float = 200.0,
        ...
        self.burning_time_left: int = (
            burning_time_left  # if set to -1, burn indefinitely
        )
...
class FireStatus(ObjectStatus):
    def __init__(self, idx, position, size, constants=default_const):
        super().__init__(
            ...
            burning_time=-1,
            burning_time_left=-1,
            temperature=constants.FIRE_TEMPERATURE,
...
class AgentStatus(ObjectStatus):
    def __init__(
        self, idx, position, size, endurance: float = 60.0, constants=default_const
    ):
        super().__init__(
            idx,
            constants,
            ...
            temperature_threshold=endurance,
...
class TemperatureManager:
    ...
    def evolve(self, objects: Dict[int, ObjectStatus]):
        ...
        for idx in objects:
            ...
            new_temperatures[idx] = (
                obj.temperature * (1 - self.decay_rate)
                + sum_weighted_temperature / sum_weight * self.decay_rate
            )
        return new_temperatures

    def query_point_temperature(
        self, target: np.ndarray, objects: Dict[int, ObjectStatus]
    ):
        sum_weight = np.exp(-1)
        sum_weighted_temperature = self.room_temperature * sum_weight
        for idx in objects:
