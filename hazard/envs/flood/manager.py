...
import numpy as np
from hazard.envs.flood.object import FloodManager, ObjectStatus
...
class FloodObjectManager(AddOn):
    def __init__(
        ...
        self.flood_manager = FloodManager(
            ascending_speed=constants.ASCENDING_SPEED,
    ...
    def update_visual_effects(self):
            ...
            floor_flood_commands.append(
                {
                    ...
                    "scale_factor": {
                        "x": new_effect_dict[floor_effect_idx]["scales"]["x"],
            ...
            floor_flood_commands.append(
                {
                    ...
                    "position": {
                        "x": new_effect_dict[floor_effect_idx]["positions"]["x"],
        ...
        for idx in self.objects:
            ...
            status, buoyancy_scale = self.flood_manager.update_object_status_new(
                self.objects[idx]
            )
            if (
                status.state == ObjectState.FLOODED
                or status.state == ObjectState.FLOODED_FLOATING
            ):
                self.objects_flooded.add(idx)
            if (
                status.state == ObjectState.FLOATING
                or status.state == ObjectState.FLOODED_FLOATING
            ):
                ...
                #     })
            # add buoyancy
            if isinstance(obj.size, np.ndarray) and isinstance(
                obj.velocity, np.ndarray
            ):
                counter += 1
                horizontal_force = self.flood_manager.cal_horizontal_force(
                    object=self.objects[idx], source=self.flood_manager.source_position
                )
                floor_flood_commands.append(
                    {
                ...
                floor_flood_commands.append(
                    {
                        "$type": "apply_force_to_object",
                        "id": status.idx,
                        "force": {"x": 0, "y": buoyancy_scale, "z": 0},
                    }
                )
        # print(counter)
            ...
            if r_id == "tran":
                ...
                for j in range(tran.get_num()):
                    ...
                    if idx in self.objects:
                        ...
                        print(
                            "Warning: object with id {} not found in FloodObjectManager".format(
                                idx
                            )
                        )
                        self.add_object(
                            ObjectStatus(idx, position=tran.get_position(j))
                        )
            elif r_id == "boun":
                boun = Bounds(resp[i])
                for j in range(boun.get_num()):
                    ...
                    if idx in self.objects:
                        ...
                        self.objects[idx].size = np.max(concat, axis=0) - np.min(
                            concat, axis=0
                        )
                ...
                for j in range(rigi.get_num()):
                    ...
                    if idx in self.objects:
                        ...
                        self.add_object(
                            ObjectStatus(idx, velocity=rigi.get_velocity(j))
                        )
        for i in range(len(resp) - 1):
            r_id = OutputData.get_data_type_id(resp[i])
            if r_id == "segm":
