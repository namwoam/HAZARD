...
from hazard.envs.flood.object import ObjectStatus
from hazard.envs.flood.agent import *
from hazard.envs.flood.flood import FloorFlood
from hazard.envs.flood.flood import FloodController
...
class FloodAgentController(FloodController):
    ...
    def update_replicant_url(self):
        if not os.path.isfile(
            f"{os.getcwd()}/data/assets/replicant_0"
        ):  # There is no local model of replicant
    ...
    def init_scene(self, setup: SceneSetup):
        ...
        if self.single_room:
            ...
            for i, room in enumerate(rooms):
                ...
                self.commands.append(
                    Controller.get_add_visual_effect(
                        name=effect_name,
                        effect_id=floor_id,
                        position=position,
                        rotation=rotation,
                ...
                self.commands.append(
                    {
                        ...
                        "scale_factor": {
                            "x": room.bounds[0] / original_effect_size[0],
            ...
            self._floors = FloorplanFlood._FLOOD_DATA[
                setup.scene_name.split("_")[-1][0]
            ]
        ...
        if not self.record_only:
            ...
            self.container_name = (
                "backpack"
                if "container" not in setup.other
                else setup.other["container"]
            )
            self.container_id = self.get_unique_id()
            self.manager.add_object(
                ObjectStatus(idx=self.container_id, waterproof=True, has_buoyancy=False)
            )

            agent_pos = self.agents[0].dynamic.transform.position
            commands.append(
                self.get_add_object(
                    model_name=self.container_name,
            ...
            self.communicate(commands)
            self.agents[0].grasp(
                target=self.container_id, arm=Arm.left, axis=None, angle=None
            )
            self.next_key_frame()
            self.agents[0].reach_for(
                target=TDWUtils.array_to_vector3([-0.3, 1.0, 0.3]),
                absolute=False,
                arrived_at=0.1,
                arm=Arm.left,
            )
    ...
    def next_key_frame(
        ...
        if force_direction is not None:
            ...
            if self.manager.flood_manager.query_point_underwater(position):
                ...
                force_scale = (
                    force_direction * (facing / np.linalg.norm(facing))
                ).sum()
                action_slowdown += (
                    1 - force_scale
                ) * self.constants.AGENT_DRAG_COEFFICIENT
    ...
    def communicate(self, commands: Union[dict, List[dict]]) -> list:
        ...
        if self.initialized:
            for target in self.target_ids:
                if (np.abs(self.manager.objects[target].position).sum() > 50) and (
                    target not in self.finished
                ):
    ...
    def get_flood_height_observation(self, idx, width=512, height=512):
        depth = TDWUtils.get_depth_values(
            self.agents[idx].dynamic.images["depth"], width=width, height=height
        )
        depth = np.flip(depth, axis=0)
        camera_matrix = self.agents[idx].dynamic.camera_matrix

        point_cloud = TDWUtils.get_point_cloud(
            depth=depth, camera_matrix=camera_matrix, vfov=120.0
        )
        ...
        for i in range(16):
            for j in range(16):
                flood_height = self.manager.query_point_flood_height(
                    point_cloud[:, i, j]
                )
    ...
    def _obs(self, agent_idx: int = 0):
        ...
        flood_height = self.get_flood_height_observation(
            agent_idx, width=self.screen_size, height=self.screen_size
        )
        flood_height = flood_height.reshape(
            (1, self.screen_size, self.screen_size)
        ).astype(np.float32)
        obs["raw"] = dict(
            rgb=rgb, depth=depth, log_temp=flood_height, seg_mask=seg_mask
        )
        ...
        for i, target_pos in enumerate(target_poss):
            ...
            if (
                ...
                continue
            if not (
                sem["id"] == self.manager.get_renumbered_id(self.target_ids[i])
            ).any():
                continue
