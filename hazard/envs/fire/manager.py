...
import numpy as np
from hazard.envs.fire.object import *
...
class FireObjectManager(AddOn):
    ...
    def get_initialization_commands(self) -> List[dict]:
        return [{"$type": "send_bounds"}, {"$type": "send_transforms"}]
            ...
            if r_id == "tran":
                ...
                for j in range(tran.get_num()):
                    ...
                    if idx in self.objects:
                        ...
                        print(
                            "object {} not recorded, may be caused by composite objects which you can ignore".format(
                                idx
                            )
                        )
                        self.add_object(
                ...
                for j in range(repl.get_num()):
                    ...
                    if idx in self.objects:
                        ...
                        print(
                            "agent {} not recorded, this shouldn't happen".format(idx)
                        )
                        self.add_object(
                            AgentStatus(idx, position=repl.get_position(j), size=None)
                        )
        for i in range(len(resp) - 1):
            r_id = OutputData.get_data_type_id(resp[i])
            if r_id == "segm":
